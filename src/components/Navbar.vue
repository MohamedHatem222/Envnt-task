<template>
  <v-app-bar app color="primary darken-1" dense dark>
    <v-btn text dark href="/">
      <img src="../../src/assets/envnt.co_.png" alt="" style="max-width:100px">
    </v-btn>
    <v-spacer></v-spacer>
        ...
        <v-tooltip disabled>
          <template v-slot:activator="{ on: tooltip }">
            <v-btn text dark v-on="{ ...tooltip, ...menu }">
              ...
              <v-badge :content="cart.length.toString()" color="primary darken-4" inline></v-badge>
            </v-btn>
          </template>
          <span>Cart</span>
        </v-tooltip>
      ...
      <v-card>
        <v-list v-if="cart.length>0">
          <v-list-item v-for="(product, index) in cart" :key="index">
            <v-list-item-avatar>
              <img :src="product.images[0]" :alt="product.image_title" />
            </v-list-item-avatar>
            <v-list-item-content>
              <v-col cols="8">
